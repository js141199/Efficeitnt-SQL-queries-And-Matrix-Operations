...
Page::Page(int pageIndex, string matrixName)
{
    logger.log("Page::Page matrix overloaded");
    ...
    ifstream fin(pageName, ios::in);
    this->rowCount = matrix.perBlockDim[pageIndex].first;
    this->columnCount = matrix.perBlockDim[pageIndex].second;
...
{    
    logger.log("Page::getRow started");

    vector<int> result;
    result.clear();
    if (rowIndex >= this->rowCount)
        return result;
    logger.log("Page::getRow ended");
